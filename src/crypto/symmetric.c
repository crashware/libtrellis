...
/* Inline dispatch: check hardware availability once per call.
 * The check is a single load of a cached flag set by sodium_init(), so it
 * is effectively free after the first call. */
static inline bool use_aes(void)
{
                                        ...
                                        uint8_t *tag)
{
    ...
    if (use_aes()) {
        rc = crypto_aead_aes256gcm_encrypt_detached(
            ciphertext, tag, &mac_len,
    ...
    return (rc == 0) ? TRELLIS_OK : TRELLIS_ERR_ENCRYPT;
}
                                        ...
                                        uint8_t *plaintext)
{
    ...
    return (rc == 0) ? TRELLIS_OK : TRELLIS_ERR_DECRYPT;
}
